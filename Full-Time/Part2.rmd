...
Due to time limitation, I will randomly sample 2000 rows for original data set.
...
set.seed(218)
index = createDataPartition(newdata$wage, p = 0.3, list = FALSE)
train = mydata[index, ]
test = mydata[-index, ]
...
### K-Nearest Neighbors
I tuned the parameter `k` with 10 values, and visualize the predicted RMSE versus the number of neighbors.
...
# RMSE
knn_rmse = rmse(predict(knn, newdata = test), test$wage)
knn_rmse
```

It's obvious that training RMSE decreases as the the number of neighbors increases. The best result given by this training process has `k = 23`.

For the algorithms below, I won't train model with `cv_3` due to the limitation of time and memory.

### Support Vector Machine (Linear Kernel)
I also tried SVR with radial kernel and produced little improvement. It sounds reasonable to me because I don't see any non-linear relationship in our training set as we have many categorical predictors.

```{r, message=FALSE, warning=FALSE}
# SVM with a linear kernel
svm = train(wage ~ ., data = train[1:500, ], 
           method = "svmLinear", 
           verbose = FALSE,
           na.action = na.pass)

# RMSE
svm_rmse = rmse(predict(svm, newdata = test), test$wage)
svm_rmse
```

### Simple Tree Model
I tuned the parameter `maxdepth` with caret library.

```{r}
# SVM with a linear kernel
ctree = train(wage ~ ., data = train[1:500, ], 
              method = "ctree2", 
              verbose = FALSE,
              tuneGrid = expand.grid(maxdepth = seq(15, 50,5)), 
              na.action = na.pass)
plot(ctree)

# RMSE
ctree_rmse = rmse(predict(ctree, newdata = test), test$wage)
ctree_rmse
```
...
```{r}
# NN
rf = train(wage ~ ., data = train[1:100, ], 
           method = "rf", 
           verbose = FALSE,
           na.action = na.pass)

# RMSE
rf_rmse = rmse(predict(rf, newdata = test), test$wage)
rf_rmse
```

### Principal Component Analysis
PCA doesn't work very well on categorical data (state, job_education,...), and there's no need to perform PCA for this analysis because we don't have that much correlated features in the dataset.
...
| Model  | RMSE |
|:----------:|:---------:|
| Linear Regression   |  `r lm_rmse`|
|KNN | `r knn_rmse`  |
| Tree |`r ctree_rmse`  |
| Support Vector Machine |`r svm_rmse`  |
| Random Forest |`r rf_rmse`  |
